    ...
    <div class="col-sm-12">
        ...
        <div class="portlet box grey">
            <div class="portlet-title">
                <div class="caption"><i class="fa fa-user"></i>编辑发布</div>
                <div class="actions">
                    ...
                    <button v-on:click="search()" class="btn btn-sm yellow filter-submit margin-bottom"><i
                                class="fa fa-search"></i> 搜索
                    </button>
                    <a v-on:click="batchfabu()" class="btn blue"><i class="fa fa-pencil"></i> 编辑发布</a>
                </div>
            </div>
                    ...
                    <tr class="odd gradeX" v-for="list in lists">
                       <!-- <td>
                        ...
                        <td>
                            ...
                            <span class="label label-sm label-success">待编辑发布</span>
                        </td>
                        <td>@{{list.status}}</td>
                        ...
                        <td>@{{list.catename}}</td>
                        <td>&nbsp;
                            <a v-on:click="remove(list.productID)" class="btn default btn-xs red"><i
    ...
    var vm = new Vue({
        ...
        methods: {
            list: function (index) {
                ...
                this.$http.get('/draftboxlist', senddata, function (data) {
                    if (data == '') {
                        ...
                        count = eval(JSON.stringify(data.last_page));
                    }
                })
            },
            search: function () {
            ...
            batchfabu: function () {
                ...
                func_action('/boxeditindex');
            },
            remove: function (id) {
                var index = layer.msg('数据更新中...', {icon: 16, time: false});
                this.$http.get('/removeeditfabu/' + id, function (data) {
                    if (eval(data).code) {
                        layer.close(index);
                        this.list(1);
                        Alert1('删除成功');
